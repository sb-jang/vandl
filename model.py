...
def get_embedding(image, post, tags, img_embedder, img_layer, post_embedder, char_embedder):
    ...
    if post != '':
        ...
        post_feature = torch.zeros(1, 300, device=device)
    ...
    if tags != '':
        ...
        individual_feature = get_post_feature(tokenized_tags, char_embedder)
        # hashtag_feature = torch.mean(torch.tensor(hashtag_feature, device=device), dim=0).squeeze(0)
        # hashtag_feature = torch.mean(hashtag_feature, dim=0)
        hashtag_feature = torch.zeros(300, device=device)
        n = individual_feature.size(0)
        for i in range(1, n+1):
            hashtag_feature += ((2*i)/(n*(n+1))) * individual_feature[i-1]
    else:
        hashtag_feature = torch.zeros(300, device=device)
